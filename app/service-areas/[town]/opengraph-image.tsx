...
export default async function Image({ params }: { params: Promise<{ town: string }> }) {
  const { town } = await params
  const area = getServiceArea(town)
  const townName = area ? area.name : 'the Flathead Valley'
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <img src={logoSrc} width={220} height={88} style={{ objectFit: 'contain' }} />
        ...
        >
          Window Cleaning in {townName}, MT
        </div>
